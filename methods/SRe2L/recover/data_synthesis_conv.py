...
def get_images(args, model_teacher, hook_for_display, ipc_id, image_syn):
    ...
    for kk in range(0, args.num_classes, batch_size):
        ...
        inputs = torch.randn((targets.shape[0], 3, 128, 128), requires_grad=True, device='cuda',
                             dtype=data_type)
        ...
        if args.store_best_images:
            ...
            for i in range(batch_size):
                image_syn.data[(kk+i)*args.ipc_end+ipc_id:(kk+i)*args.ipc_end+ipc_id+1] = best_inputs[i]
...
def parse_args():
    ...
    parser.add_argument('--verifier-arch', type=str, default='resnet18',
    ...
    parser.add_argument("--num-classes", default=10, type=int, help="dataset")
    parser.add_argument("--ranking-data-path", type=str, default="./ranking_data", help="where to store synthetic data")
...
def main_syn(ipc_id, image_syn):
    ...
    checkpoint = torch.load(
        f"./imagenet-nette_conv5.pth", map_location="cpu"
            )
    ...
    hook_for_display = lambda x,y: validate(x, y, model)
    get_images(args, model, hook_for_display, ipc_id, image_syn)


if __name__ == '__main__':
    args = parse_args()
    # for ipc_id in range(0,50):
    image_syn = torch.randn(size=(args.num_classes*args.ipc_end, 3, 128, 128), dtype=torch.float, requires_grad=False).cuda()
    for ipc_id in range(args.ipc_start, args.ipc_end):
        print('ipc = ', ipc_id)
        main_syn(ipc_id, image_syn)
    torch.save(image_syn.cpu(), os.path.join(args.ranking_data_path, "images_SRe2L_ImageNette_%d_best.pt"%( args.ipc_end)))
