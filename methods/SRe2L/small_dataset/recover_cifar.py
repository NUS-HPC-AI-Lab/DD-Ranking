...
def save_pt(args):
    ...
    image_data_tensor = torch.stack(image_data)  
    labels_tensor = torch.tensor(labels) 

    print(image_data_tensor.shape)
    os.makedirs(args.ranking_data_path, exist_ok=True)
    torch.save(image_data_tensor.cpu(), os.path.join(args.ranking_data_path, "images_%s_%d_best.pt"%(args.num_classes, args.ipc_end)))
...
def parse_args():
    ...
    args.syn_data_path = os.path.join(args.syn_data_path, args.exp_name, str(args.ipc_end))
    return args
...
if __name__ == "__main__":
    ...
    # useless for images augmentation
    # save_pt(args=args)    
