...
class ImageFolder(torchvision.datasets.ImageFolder):
    def __init__(self, classes, ipc, mem=False, shuffle=False, index=None, **kwargs):
        ...
        if self.root.split('/')[-2] == "train":
            self.index = os.listdir(self.root)
            self.index.sort(key=lambda x:int(x.split('n')[1]))
            # print(self.index)
        elif self.root.split('/')[-2] == "syn_data":
            self.index = os.listdir(self.root)
            self.index.sort(key=lambda x:int(x))
            # print(self.index)
        elif self.root.split('/')[-2] == "val":
            self.index = os.listdir(self.root)
            self.index.sort(key=lambda x:int(x.split('n')[1]))
            # print(self.index)
        ...
        for c in range(len(classes)):
            ...
            dir_path = self.root + self.index[c]
            # TinyImageNet
            # if self.root.split('/')[-2] == "train" or self.root.split('/')[-2] == "val":
            #     dir_path = self.root + self.index[c] + '/images'
...
def cutmix(images, args, rand_index=None, lam=None, bbox=None):
    ...
    # ]
    # )
    # image_np = denormalize(images[0]).data.cpu().numpy().transpose((1, 2, 0))
    # pil_image = Image.fromarray((image_np * 255).astype(np.uint8))
    # pil_image.save('./mix.png')
